...
OUTPUT_DATA_DIR = pathlib.Path(__file__).parent / "training"
ORIGINALS_DATA_DIR = pathlib.Path(__file__).parent / "training" / "originals"
...
def main():
    ...
    for filename in [
        "febrl_training_dupes_light_mod.csv",
        "febrl_training_dupes_medium_mod.csv",
        "febrl_training_dupes_heavy_mod.csv",
    ]:
    ...
    df_extra = transform.transform_febrl_dataset_without_dupes(
        ORIGINALS_DATA_DIR / "febrl_training_extras.csv"
    )
    ...
    # Shuffle rows before saving off training datasets.
    df_A = df_A.sample(frac=1).reset_index(drop=True)
    df_B = df_B.sample(frac=1).reset_index(drop=True)

    df_A.to_csv(OUTPUT_DATA_DIR / "febrl_training_a.csv", index=False)
    df_B.to_csv(OUTPUT_DATA_DIR / "febrl_training_b.csv", index=False)
    df_true_links.to_csv(OUTPUT_DATA_DIR / "febrl_training_true_links.csv", index=False)
